...
$secret = '6Ldl86IUAAAAAC-3HX6Sjq91riXtXYWcO2Uxa-V9';
...
if (isset($_POST['action']) && $_POST['action'] == 'contact_send') {
    ...
    if ($config['use_smtp'] === true) {
        ...
        $m->SMTPAuth = false;                        // enable SMTP authentication
        $m->Host = 'smtp.hostinger.com';    // sets the SMTP server
        $m->Port = 465;        // set the SMTP port for the GMAIL server
        $m->Username = 'fanny@kaleoweb.net';        // SMTP account username
        $m->Password = 'ThePrettyFanny*';            // SMTP account password
        $m->SingleTo = false;
        $m->SMTPAutoTLS = false;
        $m->CharSet = "UTF-8";
        $m->Subject = '[KaleoWeb] Demande de contact';
        $m->AltBody = 'To view the message, please use an HTML compatible email viewer!';
        $m->SMTPSecure = 'tls';
        $m->AddAddress('fanny@kaleoweb.net', '[KaleoWeb] Demande de contact');
        $m->AddReplyTo($array['email'], $array['name'] ?? $array['email']);
        $m->SetFrom('no-reply@kaleoweb.net', $array['name'] ?? $array['email']);
        $m->MsgHTML("
				{$email_body}
				---------------------------------------------------<br>
				IP: {$ip}
